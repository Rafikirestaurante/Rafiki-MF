  ...
  try {
    ...
    if (action === "login") {
      ...
      return jsonResponse(request, {
        ...
        restrictions: { movement_limit: 5, sync_days: 3, confirm_income_only: true }
      });
    }
    ...
    if (action === "list") {
      ...
      await client.from("employee_public_access_log").insert({
        ...
        detail: { count: movements.length, phase: "2B.2" }
      });
    ...
    if (action === "confirm") {
      ...
      await client.from("employee_public_access_log").insert({
        ...
        access_username: session.username,
        movement_id: movementId,
        detail: { employee_name: employeeName, duplicate_request: Boolean(confirmationError), phase: "2B.2" }
      });
